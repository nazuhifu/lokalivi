...
import { Link, router } from '@inertiajs/react'; // ⬅ tambahkan router
...
export function FeaturedProducts({ products }: { products: Products[] }) {
    const isLoggedIn = true; // TODO: ganti dengan props atau auth store jika sudah
            ...
            {products.map((product) => (
                <Card key={product.id} className="group overflow-hidden border border-gray-200 pt-0 transition-all hover:scale-105 hover:shadow-lg">
                    ...
                    <CardContent className="p4">
                        ...
                        <div className="text-2xl font-bold text-[#8B5A2B]">Rp{product.price.toLocaleString('id-ID')}</div>
                    </CardContent>
                    {isLoggedIn ? (
